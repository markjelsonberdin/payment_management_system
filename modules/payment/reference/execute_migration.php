...
try {
    ...
    $stmt = $pdo->query("SHOW COLUMNS FROM payments LIKE 'allocation_context'");
    if (!$stmt->fetch()) {
        $pdo->exec("ALTER TABLE payments ADD COLUMN allocation_context ENUM('ENROLLMENT_PRIORITY', 'SPECIFIC_ITEM') NULL AFTER category_id");
    ...
    $stmt = $pdo->query("SHOW COLUMNS FROM payments LIKE 'billing_item_id'");
    if (!$stmt->fetch()) {
        $pdo->exec("ALTER TABLE payments ADD COLUMN billing_item_id INT(10) UNSIGNED DEFAULT NULL AFTER allocation_context");
    ...
    $stmtTotal = $pdo->query("SELECT COUNT(*) FROM payments");
    ...
    $stmtHasItem = $pdo->query("SELECT COUNT(*) FROM payments WHERE billing_item_id IS NOT NULL");
