...
try {
    ...
    $affected = $pdo->exec("UPDATE payments SET allocation_context = 'ENROLLMENT_PRIORITY' WHERE allocation_context IS NULL");
    echo "   -> Updated $affected rows.\n\n";

    // 2. Validate data constraints before enforcing NOT NULL
    echo "2. Validating data constraints...\n";
    $stmtValid1 = $pdo->query("SELECT COUNT(*) FROM payments WHERE allocation_context = 'SPECIFIC_ITEM' AND billing_item_id IS NULL");
    $invalidSpecific = $stmtValid1->fetchColumn();
    
    $stmtValid2 = $pdo->query("SELECT COUNT(*) FROM payments WHERE allocation_context = 'ENROLLMENT_PRIORITY' AND billing_item_id IS NOT NULL");
    ...
    $pdo->exec("ALTER TABLE payments MODIFY COLUMN allocation_context ENUM('ENROLLMENT_PRIORITY', 'SPECIFIC_ITEM') NOT NULL DEFAULT 'ENROLLMENT_PRIORITY'");
    echo "   -> Success.\n\n";
    ...
    if (!$stmt->fetch()) {
        $pdo->exec("ALTER TABLE payments ADD CONSTRAINT fk_payments_billing_item FOREIGN KEY (billing_item_id) REFERENCES billing_items(billing_item_id) ON DELETE SET NULL");
    ...
    $stmtSummary = $pdo->query("SELECT allocation_context, COUNT(*) as count FROM payments GROUP BY allocation_context");
